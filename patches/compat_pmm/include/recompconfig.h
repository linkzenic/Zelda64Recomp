...
RECOMP_IMPORT(".", unsigned long recomp_android_compat_pmm_get_config_u32(const char* key));
RECOMP_IMPORT(".", char* recomp_android_compat_pmm_get_config_string(const char* key));
RECOMP_IMPORT(".", void recomp_android_compat_pmm_free_config_string(char* str));

static inline unsigned long recomp_get_config_u32(const char* key) {
    return recomp_android_compat_pmm_get_config_u32(key);
}

static inline double recomp_get_config_double(const char* key) {
    (void)key;
    return 0.0;
}

static inline char* recomp_get_config_string(const char* key) {
    return recomp_android_compat_pmm_get_config_string(key);
}

static inline void recomp_free_config_string(char* str) {
    recomp_android_compat_pmm_free_config_string(str);
}

static inline unsigned char* recomp_get_mod_file_path() {
    return (void*)0;
}
